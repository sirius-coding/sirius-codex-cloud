...
app = FastAPI(title="Subscription Billing API", version="1.0.0")
...
@app.post("/api/v1/subscriptions", response_model=SubscriptionRead, dependencies=[Depends(verify_token)])
...
@app.get("/api/v1/subscriptions", response_model=list[SubscriptionRead], dependencies=[Depends(verify_token)])
def list_records(
...
@app.get("/api/v1/subscriptions/{record_id}", response_model=SubscriptionRead, dependencies=[Depends(verify_token)])
...
@app.patch("/api/v1/subscriptions/{record_id}", response_model=SubscriptionRead, dependencies=[Depends(verify_token)])
def update_record(
    record_id: int,
