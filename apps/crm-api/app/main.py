...
def get_settings() -> Settings:
    return Settings(
        app_name=os.getenv("APP_NAME", "CRM API"),
...
@app.post(f"{settings.api_prefix}/customers", response_model=CustomerRead, dependencies=[Depends(verify_token)])
...
@app.get(f"{settings.api_prefix}/customers", response_model=list[CustomerRead], dependencies=[Depends(verify_token)])
def list_records(
...
@app.get(f"{settings.api_prefix}/customers/{{record_id}}", response_model=CustomerRead, dependencies=[Depends(verify_token)])
...
@app.patch(f"{settings.api_prefix}/customers/{{record_id}}", response_model=CustomerRead, dependencies=[Depends(verify_token)])
def update_record(record_id: int, payload: CustomerUpdate, session: Session = Depends(get_session)) -> Customer:
    record = session.get(Customer, record_id)
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
