...
class CustomerApiTest(unittest.TestCase):
    ...
    def test_crud_flow(self) -> None:
        ...
        list_response = self.client.get("/api/v1/customers", headers=self.headers)
        self.assertEqual(list_response.status_code, 200)
        self.assertEqual(len(list_response.json()), 1)

        detail_response = self.client.get(f"/api/v1/customers/{record_id}", headers=self.headers)
        self.assertEqual(detail_response.status_code, 200)
        self.assertEqual(detail_response.json()["name"], "示例记录")

        update_response = self.client.patch(
            f"/api/v1/customers/{record_id}",
            headers=self.headers,
